...
public class UpdatePaymentStatus extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println("OrderId,paymentId ans status recieved :" + razorpayOrderId + ","+ paymentStatus);
        // Initialize OrderDao
        OrderDao orderDao = new OrderDao(FactoryProvider.getFactory());

        // Fetch the order using the Razorpay order ID
        Orders order = orderDao.getOrderByRazorPayOrderId(razorpayOrderId);

        if (order != null) {
            // Update the order with the payment information
            order.setPaymentStatus(paymentStatus);
            order.setRazorpayOrderId(razorpayOrderId);
            order.setPaymentId(paymentId);// Assuming this field is updated
            // You can also set the paymentId if needed, if you add it as a field in Orders
